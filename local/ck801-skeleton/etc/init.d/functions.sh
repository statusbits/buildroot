...
#
# Functions to safely mount and unmount the bootblock partition.
# These allow us to only mount the bootblock one time no matter
# how many scripts need to look into it.
#
# umount_bootblock should only be inoked once, in S99umount_bootblock 
#


mount_bootblock() {
    # only mount if it hasn't already been mounted
    df /dev/mtdblock4 &> /dev/null
    if [ $? = 1 ]; then
	mount -t jffs2 /dev/mtdblock4 /etc/bootblock
    fi
}

umount_bootblock() {
    # only unmount if it has been mounted
    df /dev/mtdblock4 &> /dev/null
    if [ $? = 0 ]; then
